...
- **`search_cxql_docs` tool / `cxqldocs.go`**: the CxQL API reference (CxList method syntax, parameters, exceptions, examples) is embedded as a static JSONL file at `mcp/backend/cxqldocs/cxql_api_docs.jsonl` via `go:embed`, and searched in-process with simple keyword scoring (`MCPBackend.SearchCxQLDocs`/`FormatCxQLDocResults`) — there is no live RAG/vector-DB backing it. The JSONL was generated by a separate, unrelated pipeline (PDF → markdown → chunked JSONL) that lives outside this repo; regenerating or updating it requires going back to that pipeline, not anything in `cxqlmcp`.
